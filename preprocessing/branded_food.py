...
def process_branded_food(data_paths):
    ...
    # Rename columns
    branded_foods.rename(columns={'serving_size': 'portion_amount', 'serving_size_unit': 'portion_unit', 'household_serving_fulltext': 'portion_modifier', 'branded_food_category': 'branded_category'}, inplace=True)
    ...
    branded_foods['branded_category'] = branded_foods['branded_category'].fillna('NA').astype(str)
    ...
    # branded_foods Columns:   ['fdc_id', 'brand_owner', 'brand_name', 'ingredients', 'portion_amount', 'portion_unit', 'portion_modifier', 'branded_category']
    ...
    full_foods_merged.columns = pd.merge(foods_and_nutrients_merged, branded_foods, on='fdc_id', how='left')
    ...
    full_foods_agg = full_foods_filtered.groupby(['food_description', 'category', 'nutrient_name', 'nutrient_unit', 'portion_modifier', 
                 'portion_unit', 'brand_owner', 'brand_name', 'ingredients', 'branded_category']).mean(numeric_only=True).reset_index()
    ...
    full_foods_pivot = pd.pivot_table(full_foods_agg,
                                    index=['fdc_id', 'food_description', 'category', 'nutrient_amount', 'nutrient_unit', 'brand_owner', 
                                        'brand_name', 'ingredients', 'portion_amount', 'portion_unit', 'portion_modifier', 'branded_category'],
